...
def load_training_materials():
    """Load training materials from Excel file with detailed logging"""
    global training_materials_db
    log_file_path = "/tmp/training_load_log.txt"
    
    def log_message(msg):
        with open(log_file_path, "a") as f:
            f.write(f"{datetime.now().isoformat()} - {msg}\n")
        print(msg) # Also print to console for Vercel build logs

    log_message("Attempting to load training materials...")
    try:
        # On Vercel, files are typically in /var/task/
        possible_paths = [
            "HowToLibrary.xlsx",  # Relative path if in the same directory as index.py
            "api/HowToLibrary.xlsx", # If api is a subfolder
            "../HowToLibrary.xlsx", # If index.py is in api/ and HowToLibrary.xlsx is in root
            "/var/task/HowToLibrary.xlsx", # Common Vercel path
            "/var/task/api/HowToLibrary.xlsx" # If api is a subfolder in var/task
        ]
        
        excel_path = None
        for path in possible_paths:
            full_path = os.path.join(os.getcwd(), path) # Try relative to current working directory
            if os.path.exists(full_path):
                excel_path = full_path
                log_message(f"Found Excel file at relative path: {path} (Full path: {full_path})")
                break
            elif os.path.exists(path): # Try absolute path if provided
                excel_path = path
                log_message(f"Found Excel file at absolute path: {path}")
                break
            else:
                log_message(f"Did not find Excel file at: {path} (Checked full path: {full_path})")
        
        if excel_path:
            log_message(f"Attempting to read Excel file from: {excel_path}")
            df = pd.read_excel(excel_path)
            training_materials_db = []
            
            for _, row in df.iterrows():
                material = {
                    "Tutorial Title": str(row.get("Tutorial_Title", "")),
                    "Article Link": str(row.get("Tutorial_URL", "")),
                    "Video": str(row.get("Video_URL", "")),
                ...
                training_materials_db.append(material)
            
            log_message(f"Successfully loaded {len(training_materials_db)} training materials from Excel at {excel_path}")
        else:
            log_message("Excel file not found in any expected location. Using default materials.")
            training_materials_db = [
                {
                    "Tutorial Title": "Default Tutorial 1",
                    "Article Link": "https://example.com",
                    "Video": "https://example.com/video",
                    "Categories": "General",
                    "Tags": "tutorial, basic",
                    "Application": "General",
                    "Description": "Default tutorial description",
                    "Difficulty": "Beginner"
                }
            ]
            
    except Exception as e:
        log_message(f"Error loading training materials: {str(e)}")
        training_materials_db = [
            {
                "Tutorial Title": "Fallback Tutorial",
                "Article Link": "https://example.com",
                "Video": "https://example.com/video",
                "Categories": "General",
                "Tags": "tutorial, fallback",
                "Application": "General",
                "Description": "Fallback tutorial description",
                "Difficulty": "Beginner"
            }
        ]

# Load training materials on startup
...
@app.route('/api/training/materials/reload', methods=['POST'])
def reload_training_materials():
    auth_header = request.headers.get('Authorization')
    if not auth_header or not auth_header.startswith('Bearer '):
        return jsonify({"success": False, "message": "No token provided"}), 401
    ...
    load_training_materials()
    return jsonify({"success": True, "message": f"Reloaded {len(training_materials_db)} training materials"})
