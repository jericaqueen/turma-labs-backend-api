...
def load_training_materials():
    """Load training materials from Excel file"""
    global training_materials_db
    try:
        # On Vercel, files are typically placed in the same directory as the lambda function
        # when using a structure like api/index.py and HowToLibrary.xlsx at the root.
        # The current working directory for the lambda is /var/task/.
        excel_path = os.path.join(os.path.dirname(__file__), "HowToLibrary.xlsx")
        
        # If HowToLibrary.xlsx is at the root of the Vercel project, it will be in /var/task/
        # and index.py is in /var/task/api/, so we need to go up one level.
        if not os.path.exists(excel_path):
            excel_path = os.path.join(os.path.dirname(os.path.dirname(__file__)), "HowToLibrary.xlsx")
        ...
        if os.path.exists(excel_path):
            ...
            print(f"Successfully loaded {len(training_materials_db)} training materials from Excel at {excel_path}")
        else:
            print("Excel file not found in expected Vercel locations. Using default materials.")
            training_materials_db = [
                {
        ...
        print(f"Error loading training materials: {str(e)}")
        training_materials_db = [
            {
